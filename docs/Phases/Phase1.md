...
### Day 1 — Branch & Patch
- [x] Create and switch to `phase1-tts-engine`
- [x] Implement the `torch.load` monkey-patch in `tts_engine.py`
- [x] Write the minimal call to `ChatterboxTTS.from_pretrained(device="cpu")`
...
- [x] Add a small sanity test in `tests/` that calls the loaded model with a short, hardcoded sentence — default voice, no cloning
- [x] Save the output as a `.wav` file
- [x] Listen to it
...
- [x] Time how long generation takes for that test sentence on this exact machine
- [x] Log the result somewhere durable — a real first-hand number for this hardware, not someone else's benchmark
...
- [x] Update ARCHITECTURE.md with anything new found this phase
- [x] Update this Phase1.md checklist
...
- [x] Model loads on CPU without the `torch.load` error
- [x] A test clip generates and plays back correctly
- [x] Generation time baseline recorded for this machine
